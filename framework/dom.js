...
const changed = (oldNode, newNode) => {
    let res = false
    if (typeof oldNode != typeof newNode || (typeof oldNode == "string" && oldNode != newNode) || oldNode.type != newNode.type) {
        res = true
    }
    if (JSON.stringify(oldNode.props) !== JSON.stringify(newNode.props)) res = true;
    return res
}
...
const patch = (parent, oldNode, newNode, index = 0) => {
    if (!oldNode) {
        ...
        for (let i = 0; i < oldLen || i < newLen; i++) {
            patch(parent.childNodes[index],oldNode.children[i],newNode.children[i],i)
        }
    }
}
